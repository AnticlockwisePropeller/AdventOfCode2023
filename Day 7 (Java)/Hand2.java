...
//In this version, J cards are treated as jokers
...
public class Hand implements Comparable<Hand> {
    ...
    private void SetValues() {
        ...
        for (int i = 0; i < 5; i++) {
            switch ( cards.charAt(i) ) {
                ...
                case 'J': this.values[i] = 1; break; //J is now joker
    ...
    private void SetType() {
        ...
        //If there are jokers, and jokers are not the only kind, add them to the non-joker kind with the highest copies
        if (kindFaces.size() > 1 && kindFaces.contains(1)) {
            //Get the index of the jokers and the number of copies
            int index = kindFaces.indexOf(1);
            int copies = kindCopies.get(index);

            //Remove the jokers from the lists
            kindFaces.remove(index);
            kindCopies.remove(index);

            //Find the remaining kind with the most copies
            int max = 0;
            for (int i = 0; i < kindCopies.size(); i++) {
                if (kindCopies.get(i) > max) {
                    max = kindCopies.get(i);
                    index = i;
                }
            }
            //Add the jokers to the number of copies
            kindCopies.set(index, kindCopies.get(index) + copies);
        }
